...
function init(bgImageArg){
  if (null != bgImageArg){
	// set URL for image
       setCanvasImage(bgImageArg);
  }
...
// set 
function setCanvasImage(bgImageUrl){
	bgImage = new Image();
...
function redraw(){
  clearCanvas();
  if (null != bgImage){
	  var imageOffsetX =0, imageOffsetY=0;
	  context.drawImage(bgImage, imageOffsetX, imageOffsetY);
  }
